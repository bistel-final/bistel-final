...
function RunActionCard({ action, consec, rules }) {
  const basis = consec
        ...
        {action ? (
          <>
            <div className="flex items-center gap-3">
              <Badge variant={CODE_VARIANT[action.action_code] ?? 'bg-gray'}>{action.action_code}</Badge>
              <span className="text-[12.5px] font-bold text-ink">
                심각도 <span className={severityClass(action.severity)}>{action.severity}</span>
              </span>
            </div>
