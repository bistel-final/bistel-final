...
// incident 는 (lot_id, chamber_id) 두 개뿐 — sensor_id·equipment_id·기간은 run 의 형제 필드다
function RunContextBar({ run, equipmentId }) {
  const inc = run.incident ?? {}
  // 설비는 응답 값 우선, 없으면 챔버 ID 규칙(PHO-01-C1 → PHO-01)에서 유도
  const equipment = run.equipment_id ?? equipmentId ?? (inc.chamber_id ? inc.chamber_id.replace(/-C\d+$/, '') : null)

  // 기간은 incident 실측 incident_first_at~incident_last_at — 같은 날이면 뒤쪽은 시각만 남긴다
  const first = fmtShort(run.incident_first_at)
  const last = fmtShort(run.incident_last_at)
  const sameDay = first.slice(0, 5) === last.slice(0, 5)
  const period = first && last ? `${first} ~ ${sameDay ? fmtTime(run.incident_last_at) : last}` : DASH

  return (
    <div className="flex items-center gap-9 rounded-[10px] bg-navy px-6 py-4 text-white">
      <span className="font-mono text-[15px] font-extrabold">{run.agent_run_id}</span>
      <Badge variant={STATUS_VARIANT[run.status] ?? 'bg-gray'}>{run.status}</Badge>
      <Kv k="설비" v={equipment} />
      <Kv k="챔버" v={inc.chamber_id} />
      <Kv k="파라미터" v={run.sensor_id} />
