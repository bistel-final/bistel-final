    ...
    <>
      ...
      <div className="rounded-lg border border-tint-red-line bg-row-red p-4">
        <div className="flex items-baseline gap-3">
          ...
          <span>
            <span className="block text-[14.5px] font-extrabold text-ink">{fault?.name ?? '실측 미제공'}</span>
            <span className="mt-0.5 block font-mono text-[10px] text-g1">{fault?.name_en ?? ''}</span>
          </span>
        </div>
        <div className="mt-3 text-[12.5px] text-ink">{fault?.cause ?? '원인 문구 실측 미제공'}</div>
