...
// 전체 화면은 wafer를 LOT별 수량 node로 접고, LOT을 선택했을 때만 실제 wafer를 펼친다.
// 원본 lot_history row는 그대로 유지되며 이 함수는 read model의 화면 projection만 만든다.
export function buildLotContextGraph(graph, selectedLotId = '', selectedChamberId = '') {
  const normalized = normalizeOntologyGraph(graph)
  if (!normalized) return null
  // Chamber 단계에서는 같은 설비의 형제 Chamber를 함께 보지만, LOT까지 선택하면
  // incident 범위를 선택 Chamber 하나로 좁히고 인접 공정은 표시하지 않는다.
  const staticGraph = staticGraphForChamber(
    normalized,
    selectedChamberId,
    Boolean(selectedChamberId && !selectedLotId),
    !selectedLotId,
  )
  // 첫 진입은 정적 설비 ontology만 보인다. 생산 이력은 사용자가 LOT을 선택했을 때만 추가한다.
  if (!selectedLotId) {
    return {
    ...
    for (const [waferIndex, wafer] of wafers.entries()) {
      placed.push({
        ...
        position: {
          ...
          y: startY + 144 + Math.floor(waferIndex / waferColumns) * rowGap,
        },
      })
    }
    return placed
...
const lotIncidentLayout = (graph) => {
  ...
  placed.push(...stack(byLabel.EquipmentModel, { x: 0, centerY: 70, gap: 120 }))
  placed.push(...stack(byLabel.ProcessStep, { x: 230, centerY: 70, gap: 120 }))
  placed.push(...stack(byLabel.Area, { x: 460, centerY: 70, gap: 120 }))
  placed.push(...stack(byLabel.Equipment, { x: 690, centerY: 70, gap: 120 }))
...
export function orientOntologyRelationships(graph, layout) {
  ...
  return (normalized?.relationships ?? []).map((relationship) => {
    ...
    if (relationship.type === 'PROCESSED_IN' && sourceNode?.label === 'Lot' && targetNode?.label === 'Chamber') {
      return {
        ...relationship,
