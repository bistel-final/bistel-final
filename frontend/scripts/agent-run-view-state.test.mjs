...
assert.match(executionFlowSource, /function ToolPlanNode/, '현재 입력과 조사 루프 재호출은 Tool 노드의 서로 다른 handle로 연결해야 합니다')
assert.match(executionFlowSource, /targetHandle: 'left'/, '조사 루프 재호출선이 입력 알람 노드를 침범하면 안 됩니다')
...
assert.match(executionFlowSource, /근거 일부 부족 · 추가 수집/, '근거 부족 시 Tool 재선택 루프를 표시해야 합니다')
...
assert.match(executionFlowSource, /function ReactLoopPanel/, 'Level 3 상시 운영에서는 조사 루프 노드가 실제 조사 기록을 보여야 합니다')
assert.doesNotMatch(executionFlowSource, /EXPERIMENT|비교 실험 확장/, 'Level 3 전환 후에는 실험 확장 표기를 남기지 않습니다')
assert.match(executionFlowSource, /detail\.react_trace/, '조사 루프 패널은 저장된 ReAct 기록을 근거로 표시해야 합니다')
...
assert.match(runSummarySource, /영향 범위:/, '영향 범위는 권고 조치 안에 한 줄 요약으로 둡니다')
