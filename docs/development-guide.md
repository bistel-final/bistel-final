...
`Closes #12`를 작성한 PR이 `main`에 병합되면 연결된 Issue가 자동으로 종료됩니다. `Fixes`, `Resolves`도 같게 동작합니다.

Issue 없이 진행할 수 있는 경우는 1장 「언제 Issue 없이 진행해도 되나」를 따릅니다. 그때는 먼저 Issue를 만들고 번호를 연결합니다.
