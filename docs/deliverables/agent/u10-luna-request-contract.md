...
```json
{
  ...
  "selector_prompt_version": "agent-react-v2-ko3",
  "temperature": null,
  "seed": null,
...
U12 후속 보완부터 신규 selector는 `agent-react-v2-ko3`다. ko1·ko2 저장본은 읽지만 새 실행
admission은 실제 prompt 상수와 exact 대조하므로 ko1·ko2 설정을 거부한다. ko2·ko3는 동일한
private trace 존재·순서·호출 계수 검증을 사용하며 과거 ko2 bytes/SHA를 보존한다. 위 예시는 새 승인이 아니며
