...
> **2026-09-07 U12 초기 구현 이력(현재 ko4 보완은 아래 절)**: `agent-react-v2-ko2`에 가드 규칙·도구의 조사 목적·
...
2026-09-07 ko4의 실제 문서 신규성 정보와 정확성 우선 확대 실험은
[개발 점검 계약](development-quality-probe.md)에 기록한다. 확대 프로필은 격리
실험 전용이며 아래 U10 read8/selector10/동일 Tool4와 기존 판정식은 바꾸지 않는다.

후속 selector 메시지·선택 계약 보완은 `agent-react-v2-ko4`로 구별한다. 새 실행 admission은
현재 prompt 상수와 exact 대조하며 ko1·ko2·ko3 실행 설정을 재사용하지 않는다. 기존 ko1·ko2·ko3
artifact는 계속 읽고 원래 bytes/SHA·판정 결과를 보존한다. 이 변경 자체가 재실험 승인은 아니다.

ko3부터의 private selector 입력 계약은 production과 U10에서 같은 message builder를 사용한다.
...
ko2·ko3·ko4 artifact의 모든 attempt는 `selector_trace` 키가 필수다. REACT는 실제 trace와 ReadCall의
