...
| 영역 | 담당 | 공수 | 핵심 산출물 |
|---|---|---:|---|
| Common | 4명 공동, 통합 관리 방대혁 | 76.0h | 최종 intake·epoch·fresh bootstrap·safe graph·Runtime schema·계약·통합 gate·배포 |
...
Task 수는 104건(P2 6건 포함)이다. 대부분의 Task는 1.0~2.0h이며 예외가 서른여섯 개다.
...
- `V5-CM-4.6` **4.0h** — 6종 의존성의 marker/live 계약을 병렬 deadline으로 검사하고
...
- `V5-CM-5.2` **4.0h** — 공용 접근 없는 Mock·Kafka·topology·UI 상태 계약 1단과,
  모든 선행이 닫힌 뒤 `kosa_agent_e2e`에서 7화면·5개 확장 operation·golden flow·
  타 DB 불변·label 비누수를 실증하는 2단을 분리한다. 1단 게시를 완료 판정으로 오인하지
  않으면서 실행일 증적과 secret scan까지 한 통합 Gate로 닫는다.
...
- `V5-D-2.6` **4.0h** — 실 질의 이력 hydrate·재실행, immutable 평가 artifact 검증·조회 API와
  보조 탭을 실제 API 회귀까지 한 release 경계로 닫는다. 기존 채점 로직은 복제하지 않는다.
...
- `V5-C-5.2` **6.0h** — 공개 실행·조치 상세 read model과 저장 근거 projection,
...
- `V5-C-6.2` **4.0h** — C-6.1 원 evidence에서 baseline run 12건을 재도출하고 Runtime
...
- `V5-C-7.4` **3.0h** — 가설 반증 검증 루프(추정 · 계획리뷰에서 확정). 반증 조건 구조화 출력 prompt·Tool 매핑·가설 수정 강제·evidence additive 필드·U10 CF 회귀 재측정이 한 묶음이라 2h를 넘는다. 새 데이터·새 Tool은 없다.
...
| V5-CM-4.6 | P1 | liveness·readiness·복구. 완료: `/health`는 process 생존만 반환하고 `/health/ready`는 병렬 timeout으로 PostgreSQL epoch/schema/role·reference marker, Neo4j 44/85 marker/fingerprint, RAG 3문서·chunk·vector1024·검색 smoke, n8n, Kafka metadata·`fdc.actions`·`fdc.actions.result`와 consumer group `kosa-fdc-wf4-writeback` lag를 검사한다. 동시 요청은 하나의 판정을 공유하고 3초만 캐시하며 Kafka Admin 조회는 60초 sampler로만 수행한다. lag가 5분 이상 0보다 크거나 다른 의존성이 미준비면 sanitized 503이고 process는 종료하지 않는다 | FR-I-05, NFR-02, NFR-16 | V5-CM-4.5, V5-CM-2.7, V5-B-1.4, V5-C-4.2, V5-C-4.5 | 4.0h |
...
| V5-A-3.1 | P0 | `GET /alarms`. 완료: 최종 필터·`(source, alarm_id)`·189/192 계약, 안정 정렬과 offset 포함 시간을 제공한다. C-5.2 scaffold는 화면 조립용 임시 read만 제공하며 A가 Runtime prediction·action·delivery를 join해 `predicted_fault_code`·`action_code`·`notify_status`·`mes_status`와 호환 alias를 채우고 실제 189/192를 검증해야 완료다 | FR-A-06, NFR-11, NFR-13 | V5-A-1.5, V5-CM-4.1 | 2.0h |
| V5-A-3.2 | P0 | `GET /trace`·`GET /parameters`. 완료: 참고 React 호환 응답과 canonical field를 단일 boundary projection으로 제공하고 안정 정렬·빈 배열 계약을 지킨다. C-5.2 scaffold의 초기 route 존재는 이 Task 완료가 아니며 A 담당자가 최종 필터·DTO·실 DB 행 수를 검증해 승계한다 | FR-A-06, NFR-11, NFR-13 | V5-A-3.1 | 1.5h |
...
| V5-D-1.3 | P1 | **대체 종료(0.0h)**. run-scoped 감사 subview는 전역 Audit 화면과 목적·조회 범위가 달라 `V5-C-5.2`가 D의 bare 감사 API를 소비하는 범위로 흡수했다. 중복 구현·공수 계상은 하지 않는다 | FR-D-07, FR-I-02, NFR-17 | V5-D-1.2 | 0.0h |
...
10 나머지 A·B·C·D   실제 dependency가 열린 Task부터 병렬 구현
                    ...
                    (D-2.1~2.6은 팀 release P0 체인, D-1.3은 C-5.2에 흡수되어 대체 종료됐다)
11 V5-CM-4.4-1~4.7  7화면 shell·team release API 계약·shared client·compose·readiness·E2E reset
...
| Ontology 화면 조회 범위 | chamber 기준으로 구현(`V5-B-3.3`). 노드 타입 확장은 `GET /relations/{node_type}/{node_id}` 형태로 흡수하며 응답 스키마를 바꾸지 않는다 | 필수 7화면 완료 후 |
