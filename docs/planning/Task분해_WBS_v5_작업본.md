...
| C Agent/HITL | 방대혁 | 53.0h | LangGraph·조치·승인·n8n·Kafka·delivery·화면 3 |
| D Audit·확장 | 천승현 | 14.5h | 감사 read model·화면 3 감사 tab·선택 Text2SQL |
| **합계** | | **178.5h** | P2 도전 과제 제외 |

우선순위별 공수는 **P0 132.0h / P1 46.5h**이며 P2 3.5h는 합계에서 제외한다.
Task 수는 95건(P2 2건 포함)이다. 대부분의 Task는 1.0~2.0h이며 예외가 일곱이다.
...
- `V5-C-3.2` **4.0h** — action 4-table UoW, incident advisory lock·run row lock,
  같은 run 멱등·FAILED retry 재사용, stable request hash와 terminal provenance 보존을 실제
  PostgreSQL 동시성·rollback 회귀까지 한 저장 경계로 닫는다.
...
| V5-C-3.2 | P0 | action 생성 transaction. 완료: incident advisory lock→run row lock 아래 `action_history`·CREATED/REUSED link·approval·delivery와 policy provenance를 한 트랜잭션에서 만들고 incident당 유효 action 1건을 보장한다. `request_hash`는 stable identity의 raw 64 hex이며 같은 run 재호출과 FAILED retry를 멱등 처리한다 | FR-C-14 | V5-C-3.1 | 4.0h |
