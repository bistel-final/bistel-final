...
| V4-CM-2.2 | P0 | profile별 corrected base data 적재. 완료: `001` 성공 뒤 세 DB에 corrected source를 적재하고 action 0/0/0, PK/FK·reference output, 재실행·rollback을 검증한다 | FR-I-04 | V4-CM-2.1 | 1.0h |
| V4-CM-2.3 | P0 | evaluation Mock fixture 적재. 완료: corrected base 적재 뒤 `kosa_text2sql`에만 제공 `action_history` 48건을 추가하고 profile metadata에 `fixture_type=MOCK`을 기록한다. 이는 DB 컬럼이 아니며 Text2SQL·화면 계약 회귀에만 사용하고 Agent/Fault 정답·seed로 사용하지 않는다 | FR-D-08, FR-I-04 | V4-CM-2.2 | 1.0h |
