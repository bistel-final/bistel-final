...
> **2026-09-06 사용자 승인 변경: 신규 `MOCK-NOTIFY-V1`은 승인/반려·이메일 확인 기록 없이 이메일 알림과 Kafka/MES Mock 자동 연동만 수행한다. 아래 HITL 설명은 기존 `ACTION-POLICY-V1` 이력/증적에 한정한다. 원본 데이터는 불변이며 공용 적용은 아직 하지 않았다. [변경 계약·배포 경계](../deliverables/agent/mock-notify-contract.md)를 함께 읽는다.**
