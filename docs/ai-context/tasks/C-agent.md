...
**P0·P1 22 Task / 46.5h** · **P2 별도 1 Task / 2.0h**
...
- 동일 incident 활성 run 1개, incident당 유효 action 1개를 보장한다. FAILED retry는 새 run을
  만들되 기존 action을 `REUSED`로 연결한다.
...
- Tool 예산은 총 8회이며 interrupt 전후 누적값을 checkpoint와 DB에서 복원한다.
