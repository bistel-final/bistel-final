...
> 최종 수정일: 2026-08-12
>
> 기준 문서: `요구사항정의서_v1_9_최종.md`, `FDC_프로젝트_역할분담_v9.6(최종).md`
>
> 목표 제출일: 2026-08-14
>
...
| v1.2 | 2026-08-05 | 운영 Analytics와 평가 DB·Compose 환경 주입 분리, nullable incident key의 NOT NULL 보강, 반복 LOT·정상 복귀 판정 DTO·양성/음성 fixture, Tool 8회 단계별 예산·영속 시도 복원·전송 예약, 공용 서버 최소권한 role 4종·기본 자격증명 전환 절차, `audit_log` append-only DB 권한, Backend/n8n `request_hash` 신뢰 경계, Agent Tool 4종+독립 Analytics Tool 1종의 변경 사유 반영. 후속 정합 보정 4건: 운영·평가 DB의 `action_history` 스키마 차이에 따른 allowlist 컬럼 캐시·스키마 컨텍스트 pool별 분리(9.3·9.5), Agent E2E 초기화 대상의 `nl_query_log` 표기 통일(14.1), 2.1 목표 트리를 실제 저장소 구조로 갱신(`docs/specifications/`·`docs/ai-context/`·`CLAUDE.md`·`AGENTS.md`·`backend/pytest.ini`), 10.1 공통 Enum 의 코드 기준으로 `common/enums.py` 추가 및 `ActionCode`·`SendChannel`·`FaultCode`·`ActorType` 4종 명시 |
| v1.3 | 2026-08-08 | 7.7 조치 결정 함수에서 `anomaly_score`를 입력 목록에서 제외. 점수는 화면 표시·LLM 근거·모델 평가 전용이며 조치 코드·심각도·승인 판정에 사용하지 않는다는 근거를 명시 |
| v1.4 | 2026-08-08 | R03_CONSEC 보유 incident 에서 해당 WAFER 의 FDC 요약을 1회 추가 조회하도록 확정(State `r03_fdc_evidence`·예산 조건부 1회). 대표 알람이 R03 이 아니어서 결정적 WAFER 파형이 근거에서 빠지던 문제 해소 |
| v1.5 | 2026-08-08 | 7.6 에 `anomaly_score` 는 프롬프트에 포함되나 `fault_code` 결정 근거가 아님을 명시. 7.7 의 조치 결정 제외 규정과 짝을 맞춤 |
| v1.6 | 2026-08-08 | `IncidentAlarmEvidence.sibling_alarm_counts` 신설. 진단 가이드 3.1 이 요구하는 「특정 챔버만 이탈 vs 두 챔버 모두」 판단 근거를 `load_incident` 집계로 확보 |
| v1.7 | 2026-08-08 | 멘토링 피드백 반영 — 12장 라우트를 파라미터 중심 화면 구성으로 교체(`/dashboard`·`/traces`·`/actions`·`/agent-runs/:runId`·`/knowledge`), 트레이스 조회를 `GET /traces/catalog` + `POST /traces/search`로 분리, `GET /agent/runs`·`GET /actions` 목록 엔드포인트 신설 |
| v1.8 | 2026-08-08 | 화면이 쓰는 필드를 DTO 에 반영 — `ApprovalItem` 을 `IncidentRef` 기반으로 바꾸고 `sensor_id`·`severity` 추가, `ActionDetailResponse`·`AgentRunDetailResponse` 에 `sensor_id`·`alarm_count`·구간 시각 추가, `SqlValidateResponse` 에 `checks` 추가 |
| v1.9 | 2026-08-11 | API 명세서 v2.0 을 10장에 반영 — 대시보드 응답을 파라미터·추이 중심으로 교체(`hierarchy`·`sensor_catalog`·`daily_trend`·`top_sensors`·`equipment_counts`·`pending_approvals`)하고 `chamber_statuses`·`metrology_pass_rate` 제거, 트레이스 카탈로그·검색 DTO 신설, `AgentRunItem`·`ActionItem`·`R03EvidenceRef` 신설, `GET /analytics/history` 신설, `AnalysisQueryResponse` 에 검증·기록 필드 추가. 명세서 v2.0 의 `metric.type`·`chart_type` 표기는 요구사항정의서 v1.8 FR-D-04·8.4 와 어긋나 채택하지 않는다 |
| v1.10 | 2026-08-11~12 | 요구사항 v1.9와 API 명세서 v2.1 정렬 — 8화면 계약, Text2SQL 정책 거부 200 구조화 응답, `ActionApprovalStatus` 분리, Agent Fault Code 4종·nullability, readiness 실패 reason, `created_by_agent_run_id`, Sensor·Tool·평가·감사 DTO, 안정 정렬·입력 제약·DB/API 필드 매핑을 확정. 후속 정합 보정으로 `/health`·`/health/ready`를 22개 업무 API와 분리된 내부 운영·개발 진단 엔드포인트로 명시하고, CM-0.4 source manifest v2의 원본 컬럼·행 수·canonical hash 고정 및 read-only 검증 절차를 반영 |
...
1. 기능 동작과 수용 기준: `요구사항정의서_v1_9_최종.md`
2. 역할과 Tool 소유권: `FDC_프로젝트_역할분담_v9.6(최종).md`
...
멘토 원안과 수행계획서는 Streamlit 7화면을 기본 범위로 두고 React 풀스택을 확장 범위로 분류한다. 그러나 **2026-07-31 멘토링에서 FastAPI+React 전환을 초기 필수 범위로 확정**했고, 후속 화면 설계에서 조치 목록과 Agent 실행·승인 상세를 분리했다. 요구사항 v1.9에 따라 최종 UI는 12장의 **8화면**으로 확정한다. 도메인 기능을 새로 늘린 것이 아니라 기존 Agent 분석·승인 기능을 사용 흐름에 맞춰 두 라우트로 분리한 것이며, 이 결정은 사용자 인증·클라우드 배포·실제 MES/메일 연동까지 범위를 넓히는 근거가 아니다.
...
│   │   │   ├── config.py
│   │   │   ├── db.py
│   │   │   ├── enums.py               # 10.1 공통 Enum 의 단일 기준
│   │   │   ├── schemas.py             # Page·Incident 공통 DTO
│   │   │   ├── tool_contracts.py
│   │   │   ├── neo4j.py
│   │   │   ├── exceptions.py
│   │   │   ├── audit.py
...
└── docs/
    ...
    │       ├── C-agent.md
    │       └── D-analytics.md
    ├── deliverables/                  # 제출·공유 산출물
    │   └── api/
    │       ├── API명세서.csv           # 편집·검토용 표 형식
    │       ├── API명세서.md            # Git diff·검색용 원본
    │       ├── API명세서.pdf           # 제출·회람용 렌더링 결과
    │       ├── build_api_spec.py       # 세 형식을 함께 생성하는 단일 생성기
    │       ├── requirements.txt        # 문서 생성 전용 의존성
    │       └── README.md               # 재생성·검증 절차
...
`docs/deliverables/api/`의 CSV·Markdown·PDF는 같은 생성기로 만들며, API 계약을 변경하면 세 파일을 함께 갱신한다. Markdown과 Backend Pydantic DTO를 검토 가능한 기준으로 삼고 PDF는 제출·회람용 결과물로 사용한다.
...
| 요청 본문·쿼리 형식 오류 | 422 |
| Text2SQL 정책 위반 | 200 (`is_rejected=true` 구조화 응답) |
| 기능별 모델·LLM 산출물 미준비 | 503 |
| 예기치 못한 서버 오류 | 500 |

Text2SQL 정책 위반은 요청 형식 오류가 아니다. 생성 SQL을 실행하지 않고 `nl_query_log`에 거부 결과를 남긴 뒤 `AnalysisQueryResponse`로 HTTP 200을 반환한다. `sql`·`metric`·`visualization`은 `null`, `columns`·`rows`·`group_by`는 빈 배열, `row_count=0`, `is_valid=false`, `is_rejected=true`로 고정한다. `PolicyRejectedError`와 Tool의 `POLICY_REJECTED:` 접두어는 삭제하지 않으며, 전자는 Text2SQL HTTP 성공 경로 밖에서 명시적 정책 예외가 필요한 경우에만 사용하고 후자는 `generate_analysis_plan` Tool의 안전 실패 계약으로 유지한다.

500 응답과 애플리케이션 오류 로그에는 비밀번호, 전체 DB URL, API 키, 내부 SQL 원문에 포함된 민감정보를 노출하지 않는다. `nl_query_log.generated_sql`은 기능 요구에 따라 생성 SQL을 의도적으로 저장하는 업무 데이터이므로 이 금지의 대상인 오류 로그와 구분한다.

애플리케이션 ErrorCode는 `RESOURCE_NOT_FOUND`, `INCIDENT_ALREADY_RUNNING`, `INCIDENT_ALREADY_PROCESSED`, `APPROVAL_ALREADY_DECIDED`, `LEGACY_APPROVAL_NOT_LINKED`, `IDEMPOTENCY_CONFLICT`, `VALIDATION_ERROR`, `POLICY_REJECTED`, `DEPENDENCY_NOT_READY`, `MODEL_NOT_READY`, `LLM_NOT_READY`, `INTERNAL_ERROR`로 고정한다. 409 다섯 종은 모든 API가 공통으로 반환한다는 뜻이 아니라 각 업무 경로에 맞게 연결한다. Agent 실행 생성은 incident 실행 상태에 따라 `INCIDENT_ALREADY_RUNNING|INCIDENT_ALREADY_PROCESSED`, 승인 결정은 `APPROVAL_ALREADY_DECIDED|LEGACY_APPROVAL_NOT_LINKED`, 전송 멱등 충돌은 `IDEMPOTENCY_CONFLICT`를 사용한다. FastAPI/Pydantic 입력 검증 실패는 Body·path·query 모두 422 `VALIDATION_ERROR`로 정규화한다.
...
#### 3.2.3 조치 생성 실행·전송 시작 시각·시도 횟수

원본 `action_history`에는 조치를 최초 생성한 Agent 실행과 SENDING 진입 시각이 없어 생성 근거와 고착 시간을 정확히 확인할 수 없다. 별도 마이그레이션으로 다음 컬럼을 추가한다.

```sql
ALTER TABLE action_history
    ADD COLUMN IF NOT EXISTS created_by_agent_run_id varchar(20),
...
BEGIN
    IF NOT EXISTS (
        ...
        WHERE conname = 'fk_action_history_created_by_run'
          AND conrelid = 'public.action_history'::regclass
    ) THEN
        ALTER TABLE public.action_history
            ADD CONSTRAINT fk_action_history_created_by_run
            FOREIGN KEY (created_by_agent_run_id)
            REFERENCES public.agent_run(agent_run_id);
    END IF;
END $$;
```

- 신규 조치 생성 트랜잭션은 현재 `agent_run_id`를 `created_by_agent_run_id`에 한 번 기록한다.
- 수동 재실행이 기존 `action_id`를 재사용하면 `created_by_agent_run_id`는 최초 생성 실행을 계속 가리키며 갱신하지 않는다.
- 배포 기준 데이터의 기존 ACT-0001~ACT-0010은 대응 Agent 실행 행이 없으므로 `created_by_agent_run_id=NULL`인 legacy 조치로 유지한다. 최신 실행을 추측해 backfill하지 않는다.
...
`DashboardService`가 A의 알람 통계와 C의 `ApprovalService.list_pending()`을 조합한다. 승인 대기 SQL을 A에 복제하지 않는다.

`date_range`·`reference_date` 결정 순서:

1. `date_from`·`date_to` 모두 지정: 입력 기간을 적용한다. `date_from > date_to`는 422다.
2. 두 경계를 모두 생략: 선택한 AREA·설비·챔버 범위에서 알람이 존재하는 최소·최대 일자를 적용한다.
3. 한쪽 경계만 생략: 같은 계층 범위의 데이터 최소일 또는 최대일로 생략한 경계를 보완한다.
4. `reference_date`: 실제 적용 기간 안에서 알람이 존재하는 최신 일자다.
5. 계층 범위에 데이터 경계 자체가 없으면 `date_range=[]`, `reference_date=null`, 수치 0과 빈 목록을 반환한다. 명시한 기간에 데이터가 없으면 입력 기간은 `date_range`로 되돌려주고 `reference_date=null`로 둔다.

Repository 산식과 정렬은 다음으로 고정한다.

- 알람 수: 적용 기간·AREA·설비·챔버 범위의 `fdc_alarm` 행 수
- OOS/OOC 수: 같은 범위에서 `judgement='OOS'/'OOC'`인 알람 행 수
- 일자별 추이: 선택된 필터 범위의 날짜별 OOS·OOC 행 수와 R03_CONSEC 존재 여부. 날짜 오름차순
- 상위 파라미터: 같은 범위의 `sensor_id`별 알람 행 수를 `alarm_count DESC, sensor_id ASC`로 정렬한 상위 5건
- 설비별 건수: 같은 범위의 `equipment_id`별 알람 행 수와 챔버별 건수를 반환하며 설비·챔버 ID 오름차순으로 안정 정렬
- 승인 대기: 날짜·AREA·설비·챔버와 무관한 전체 `approval_request.status='PENDING'` 목록. `requested_at DESC, approval_id DESC`
- 최근 알람: 조회 범위에서 `occurred_at DESC, alarm_id DESC` 상위 5건

화면 필터 선택지는 `dim_process_step`·`dim_equipment`·`dim_chamber`·`dim_sensor`의 계층과 알람 데이터의 전체 날짜 범위에서 만든다. 배포 전체 계층·기간 자동 적용 회귀 기대값은 `date_range=[2026-06-01, 2026-06-04]`, `reference_date=2026-06-04`, 알람 51건·OOS 37건·OOC 14건이다. 계측 PASS율과 챔버 상태 카드는 알람 대시보드 응답 범위가 아니며, 필요한 통계는 D Analytics 자연어 질의로 조회한다.
...
class AgentState(TypedDict):
    ...
    r03_fdc_evidence: FdcSummaryToolResult | None      # R03_CONSEC 이 난 WAFER
...
**`load_incident`는 같은 장비의 형제 챔버별 알람 건수도 함께 집계해 `sibling_alarm_counts`에 담는다.** 진단 가이드 3.1은 `FOC`의 원인과 조치를 「특정 챔버만 이탈」과 「두 챔버 모두 이탈」로 나누어 제시한다(전자는 척 표면 이물 → 해당 챔버 점검, 후자는 포커스 센서 교정 확인). 이 구분은 형제 챔버의 알람 유무를 알아야 판단할 수 있는데, `get_equipment_context`가 반환하는 `sibling_chambers`는 Neo4j의 챔버 목록일 뿐 알람 건수를 담지 않는다. `load_incident`가 이미 `fdc_alarm`을 읽으므로 같은 조회에서 `equipment_id`가 같은 챔버별로 집계하면 추가 Tool 호출 없이 확보된다. 배포 데이터에서는 `PHO-01-C1` 22건 대 `PHO-01-C2` 0건으로 「특정 챔버만 이탈」에 해당한다.

**추가 호출 조건을 `R03_CONSEC` 으로 고정한다.** 대표 알람은 `occurred_at ASC` 첫 항목이라 항상 앞쪽 WAFER이고, `R03_CONSEC` 은 연속 3장째에 생성되므로 뒤쪽 WAFER다. 배포 데이터의 R03 보유 incident 3건(LOT-260004·260007·260010)은 **모두 대표 알람이 R03 이 아니다.** 대표 근거만 쓰면 `EQP_HOLD` 를 유발한 바로 그 WAFER의 센서 파형을 근거에 담지 못한다. 따라서 incident 에 `R03_CONSEC` 알람이 있으면 그 알람의 `lot_hist_id` 로 `get_fdc_summary` 를 1회 더 호출해 `r03_fdc_evidence` 에 담는다. R03 이 여러 건이면 `occurred_at ASC, alarm_id ASC` 첫 항목을 쓴다. `action_history.trigger_alarm_lot_hist_id` 는 배포 정답과 동일하게 대표 알람을 가리키며 이 추가 조회로 바뀌지 않는다.
...
| 조건부 진단 | `get_fdc_summary` 1회 추가 | incident 에 `R03_CONSEC` 이 있을 때만. 잔여 예산이 없으면 생략하고 대표 근거로 진행 |
...
- `anomaly_score`·`anomaly_threshold`·`is_anomaly`는 요약 근거에 포함되어 프롬프트에 실리지만 **`fault_code` 결정 근거로 삼지 않는다.** 이상 유형은 어느 파라미터가 어느 RECIPE STEP에서 이탈했는지로 판단하며(`code_fault.main_sensor_id`·`main_recipe_step`), 점수는 `cause_summary` 서술의 참고 정보다. 같은 파라미터·STEP이면 점수가 달라도 유형은 바뀌지 않는다.
...
`decide_action(IncidentDecisionInput) -> ActionDecision`은 순수 함수로 구현한다. 입력에는 알람 집계, 계측, 반복 LOT, 하류 전입, 다음 WAFER 정상 복귀, batch upstream 계획이 포함되며 DB·LLM에 직접 접근하지 않는다. `ActionDecision`은 `action_code:str|null`, `reason:str`, `severity:Severity|null`, `approval_required:bool`, `send_channel:EMAIL|MES|null`, `skip_action:bool`을 반환한다.

**`anomaly_score`는 `decide_action`의 입력에 포함하지 않는다.** 조치 코드·심각도·승인 여부는 7.5의 규칙만으로 결정하며, 점수는 화면 표시·LLM 분류 근거·모델 평가에만 사용한다. `R03_CONSEC`은 WAFER 여러 장에 걸친 연속 패턴인 반면 `anomaly_score`는 WAFER 한 장 단위로 산출되므로 점수와 조치 심각도가 비례하지 않는다. 점수를 등급 판정에 넣으면 가장 심각한 `EQP_HOLD` 건을 오히려 낮게 평가하게 된다(예비 학습 실험에서 확인). 배치 처리 순서나 목록 정렬 같은 판정 밖 용도로만 활용한다.
...
평가 전용 readonly·QueryLog pool은 evaluation one-shot 시작 시 각각 `SELECT 1`로 점검한다. 하나라도 실패하면 해당 실행을 시작하지 않되 자격증명이나 DSN을 응답·로그에 노출하지 않는다.
...
3. `verify_source_data.py --profile evaluation`으로 16개 평가 대상 base table의 원본 컬럼 목록·행 수·canonical content hash와 `action_history=10`을 `infra/bootstrap/source-data-manifest.json` v2와 비교한다. `nl_query_log`는 평가 이력 누적 테이블이므로 이 hash에서 제외한다. runtime migration이 추가한 컬럼은 manifest의 원본 컬럼 목록에 없으므로 source hash에서 제외하고 `verify_migrations.py`가 별도로 검증한다.
...
- Backend 직접 개발 Base URL은 `http://localhost:8000`, React·최종 배포의 공개 Base URL은 같은 Origin의 `/api`다. 아래 엔드포인트 경로는 Backend 내부 경로이며 Nginx·Vite proxy가 공개 `/api` prefix를 제거해 전달한다.
- `/health`·`/health/ready`는 배포·운영 및 개발 진단을 위한 내부 엔드포인트다. 요구사항 FR-I-05의 업무 기능, 아래 22개 API 목록, API 명세서 엔드포인트 개수와 기능 수용 기준에는 포함하지 않는다. 전자는 프로세스 생존만, 후자는 PostgreSQL·Neo4j·n8n의 정규화된 준비 상태만 반환하며 비밀번호·전체 DSN·원본 접속 오류는 노출하지 않는다.
- JSON 요청·응답은 `Content-Type: application/json; charset=utf-8`을 사용한다. 사용자 인증·JWT·RBAC는 초기 프로젝트 범위 밖이며 교육장 내부망 접근제어를 전제로 한다. n8n webhook의 `X-Webhook-Secret`은 별도 서비스 간 인증으로 유지한다.

공통 Enum — 코드 기준은 `backend/app/common/enums.py` 하나이며 도메인 schema 와 Tool 계약이 여기서 import 한다. `ActionCode` 의 severity·send_channel·승인 필요 여부는 같은 모듈의 순수 함수로 고정하고 LLM 출력으로 채우지 않는다:
...
`ChartType`과 `MetricPlan.type`은 요구사항정의서 v1.9 의 FR-D-04·8.4 가 고정한 값이며 설계서가 임의로 넓히거나 좁히지 않는다. API 명세서 v2.1도 같은 Enum을 사용한다(9.6·10.5).

`ChamberStatus`는 대시보드가 챔버 상태 카드 대신 파라미터·추이 중심 구성으로 바뀌면서(10.2) 어떤 API 계약에도 쓰이지 않아 `backend/app/common/enums.py`에서 제거한다.

IsolationForest·임베딩 모델·LLM credential이 준비되지 않은 경우 해당 기능 API만 HTTP 503 `MODEL_NOT_READY` 또는 `LLM_NOT_READY`를 반환하고, Tool은 `{ok:false, reason}`으로 안전 실패한다. 외부 의존성 오류가 FastAPI 프로세스를 종료시키지는 않는다(NFR-16).

### 10.2 A Detection API

| API | 주요 입력 | 주요 응답 |
|---|---|---|
| `GET /dashboard/summary` | `date_from?`, `date_to?`, `area?`, `equipment_id?`, `chamber_id?` | reference_date, date_range, hierarchy, sensor_catalog, alarm_count, oos_count, ooc_count, daily_trend, top_sensors, equipment_counts, pending_approvals, recent_alarms |
| `GET /summaries/{lot_hist_id}` | path | wafer, sensors, anomaly_score, threshold, is_anomaly |
| `GET /alarms` | date_from?, date_to?, area?, equipment_id?, chamber_id?, sensor_id?, rule_id?, judgement?, page=1, size=20 | items, total, page, size |
| `GET /alarms/{alarm_id}` | path | alarm 상세 + incident key + 최신 Agent 실행 연결 |
| `GET /traces/catalog` | 없음 | 조회 선택지 — area·equipment·chamber·sensor·recipe·lot·wafer 목록과 센서 한계선 |
| `POST /traces/search` | 아래 body | 파라미터별 series·한계선 5개·구간 경계 |
...
| Model | 필드와 타입 |
|---|---|
| `AlarmItem` | `alarm_id:str`, `lot_hist_id:str`, `lot_id:str`, `wafer_no:int\|null`, `chamber_id:str\|null`, `equipment_id:str\|null`, `sensor_id:str\|null`, `recipe_step_no:int\|null`, `recipe_step_name:str\|null`, `rule_id:str\|null`, `judgement:Judgement\|null`, `hit_cnt:int\|null`, `detail:str\|null`, `occurred_at:datetime`, `incident:IncidentRef`, `action_id:str\|null`, `action_code:str\|null`, `approval_status:ActionApprovalStatus\|null`, `latest_agent_run_id:str\|null`, `agent_run_status:AgentRunStatus\|null` |
| `HierarchyNode` | `area_id:str`, `equipment_id:str`, `model_code:str\|null`, `chambers:list[str]` |
| `DailyTrendItem` | `date:date`, `oos_count:int`, `ooc_count:int`, `has_r03_consec:bool` |
| `TopSensorItem` | `sensor_id:str`, `alarm_count:int`, `chamber_ids:list[str]` |
| `EquipmentCountItem` | `equipment_id:str`, `area_id:str\|null`, `alarm_count:int`, `chambers:list[{chamber_id:str,alarm_count:int}]` |
| `DashboardSummaryResponse` | `reference_date:date\|null`, `area:str\|null`, `date_range:list[date](길이 0 또는 2)`, `hierarchy:list[HierarchyNode]`, `sensor_catalog:list[str]`, `alarm_count:int`, `oos_count:int`, `ooc_count:int`, `daily_trend:list[DailyTrendItem]`, `top_sensors:list[TopSensorItem]`, `equipment_counts:list[EquipmentCountItem]`, `pending_approvals:list[ApprovalItem]`, `recent_alarms:list[AlarmItem]` |
| `WaferContext` | `lot_hist_id:str`, `lot_id:str`, `wafer_no:int`, `chamber_id:str`, `equipment_id:str`, `step_id:str`, `recipe_id:str\|null` |
| `SensorSummaryItem` | `sensor_id:str`, `sensor_name:str`, `unit:str\|null`, `recipe_step_no:int`, `recipe_step_name:str\|null`, `value_mean:float\|null`, `value_std:float\|null`, `value_min:float\|null`, `value_max:float\|null`, `point_cnt:int`, `ooc_point_cnt:int`, `oos_point_cnt:int`, `spec_lower:float\|null`, `ctrl_lower:float\|null`, `target:float\|null`, `ctrl_upper:float\|null`, `spec_upper:float\|null`, `judgement:Judgement` |
...
| `SensorLimits` | `unit:str\|null`, `spec_lower:float\|null`, `ctrl_lower:float\|null`, `target:float\|null`, `ctrl_upper:float\|null`, `spec_upper:float\|null`, `upper_only:bool` |
| `TraceCatalogResponse` | `areas:list[{area_id:str}]`, `equipments:list[{equipment_id:str,area_id:str,model_code:str\|null,chambers:list[str]}]`, `sensors:list[{sensor_id:str,sensor_name:str} + SensorLimits]`, `recipes:list[{recipe_id:str,step_id:str}]`, `lots:list[{lot_id:str,wafer_nos:list[int]}]`, `anomaly:{threshold:float}` |
| `TraceSearchRequest` | `area:str\|null`, `equipment_id:str\|null`, `chamber_id:str\|null`, `sensor_ids:list[str]`, `recipe_id:str\|null`, `lot_id:str\|null`, `wafer_nos:list[int]`, `from:datetime\|null`, `to:datetime\|null` |
| `TracePoint` | `seq_no:int`, `recipe_step_no:int\|null`, `recipe_step_name:str\|null`, `measured_at:datetime\|null`, `value:float` |
| `TraceWaferSeries` | `lot_hist_id:str`, `lot_id:str`, `wafer_no:int`, `chamber_id:str`, `equipment_id:str`, `recipe_id:str\|null`, `sensor_id:str`, `occurred_at:datetime\|null`, `points:list[TracePoint]` |
| `MeasuredStepStat` | `lot_hist_id:str`, `sensor_id:str`, `recipe_step_no:int`, `recipe_step_name:str\|null`, `value_mean:float\|null`, `value_std:float\|null`, `value_min:float\|null`, `value_max:float\|null`, `point_cnt:int`, `ooc_point_cnt:int`, `oos_point_cnt:int`, `judgement:Judgement` |
| `TraceSearchResponse` | `wafers:list[TraceWaferSeries]`, `limits:dict[sensor_id, SensorLimits]`, `measured_step_stats:list[MeasuredStepStat]`, `total:int` |

**트레이스 조회는 `POST`를 쓴다.** 조회 조건에 파라미터 다중·WAFER 다중·기간이 함께 들어가는데 query string에는 배열 표준 표기가 없어 반복 키(`sensor_id=A&sensor_id=B`)나 구분자(`sensor_id=A,B`) 중 하나를 임의로 정해야 한다. JSON body가 명확하고 `POST /documents/search`와도 일관된다. 화면 주소(`/traces?lot=...&param=...`)는 프론트 라우팅이 담당하므로 공유·북마크는 그대로 가능하다.
...
POST /traces/search
{
  ...
  "sensor_ids": ["PH_FOCUS", "PH_DOSE"],
  "recipe_id": "RCP-PH-A1",
  "lot_id": "LOT-260008",
  "wafer_nos": [1, 2, 3],
  "from": "2026-06-01T06:00:00+09:00",
  "to": "2026-06-01T07:00:00+09:00"
}
```

`GET /traces/catalog`는 선택지 목록만 반환하고 시계열을 담지 않는다. 실제 측정값은 `POST /traces/search`가 준다.

`sensor_ids`는 1개 이상이며 중복을 거부하고, `wafer_nos`는 생략 또는 빈 배열이면 전체 WAFER를 뜻하며 중복을 거부한다. `from`과 `to`를 함께 주면 `from < to`여야 하고 `fdc_trace.measured_at`의 양 끝 포함 구간으로 필터링한다. 응답 `total`은 고유 WAFER 수가 아니라 `(lot_hist_id, sensor_id)` series 수다. 각 series의 `points`는 `seq_no ASC`, `measured_step_stats`는 `lot_hist_id ASC, sensor_id ASC, recipe_step_no ASC`로 정렬한다.

**검색 응답은 센서가 아니라 WAFER 를 단위로 한다.** 화면이 「파라미터를 선택한 개수만큼 차트, LOT·WAFER 가 여러 개면 와이드하게」를 그리므로 `(WAFER, 센서)` 조합마다 한 series 를 반환하는 `TraceWaferSeries` 가 그대로 차트 1개에 대응한다. 센서 한계선은 series 마다 반복하지 않고 `limits` 맵에 센서별로 한 번만 담는다. `upper_only=true`는 하한선을 렌더링하지 않는다는 표시다. 초기 배포 도메인에서 반사파 `ET_REFL`만 `upper_only=true`, 나머지 센서는 false다. 배포 데이터의 `ET_REFL.spec_lower=0.0`, `ctrl_lower=0.0`이므로 `spec_lower IS NULL` 같은 null 판정으로 계산하지 않는다. 원본 `01_schema.sql`을 수정하지 않고 Backend의 명시적 센서 메타데이터 규칙에서 산출해 프론트가 센서 ID를 하드코딩하지 않게 한다. `measured_step_stats`는 같은 조회 범위의 `fdc_summary` 통계로, 차트 옆 표와 STEP 구간 음영에 쓴다.

`GET /alarms`는 `Page[AlarmItem]`을 `occurred_at DESC, alarm_id DESC`로 반환한다. **목록 항목과 상세 항목은 같은 `AlarmItem` 이다.** 목록에서 헤더별 필터와 「에이전트 처리상태」 열을 함께 보여주려면 `incident`·`action_*`·`latest_agent_run_id`·`agent_run_status` 가 목록 단계에서 필요하므로 상세 전용 DTO 를 두지 않는다. 최신 실행은 `agent_run_alarm`을 통해 `started_at DESC, agent_run_id DESC` 첫 행으로 결정한다.

**대시보드는 챔버 상태 카드가 아니라 파라미터·추이 중심이다.** 멘토링에서 확인한 실제 조회 습관은 「챔버 상태로는 잘 안 보고 파라미터로 본다 · 날짜별로 볼 때는 트렌드로 · 설비별로 몇 건이냐를 많이 본다 · 알람이 높은 상위 5개」다. 따라서 `chamber_statuses`와 `metrology_pass_rate`를 응답에서 제거하고 `daily_trend`·`top_sensors`·`equipment_counts`로 대체한다. 계측 패스율은 알람 대시보드의 지표가 아니며 필요하면 D Analytics 질의로 뽑는다.

`hierarchy`·`sensor_catalog`·`date_range`는 화면 필터의 선택지를 서버가 내려주기 위한 필드다. AREA·설비·챔버·파라미터를 프론트에 상수로 박지 않고 매 조회마다 서버 값으로 채워 데이터가 늘어도 화면이 따라간다. `date_range`는 `daily_trend`가 덮는 구간의 `[시작일, 종료일]` 두 개다.

`pending_approvals`는 승인 대기 건수 하나가 아니라 목록 자체다. 대기 항목이 개별 카드가 아니라 리스트로 보여야 한다는 피드백을 반영해 `ApprovalItem`을 그대로 담는다. `reference_date=null`은 필터 범위에 알람 데이터가 전혀 없을 때만 허용한다.

`date_from`·`date_to`를 모두 생략하면 선택 계층의 알람 최소·최대 일자를 자동 적용하고, 한쪽만 지정하면 생략한 경계를 해당 계층의 데이터 경계로 보완한다. `reference_date`는 적용 기간 안의 최신 데이터 일자다. `alarm_count`·`oos_count`·`ooc_count`는 distinct WAFER가 아니라 `fdc_alarm` 행 기준이다. `top_sensors`는 `alarm_count DESC, sensor_id ASC` 상위 5건, `recent_alarms`는 `occurred_at DESC, alarm_id DESC` 상위 5건이다. `pending_approvals`는 기간·AREA·설비·챔버 필터와 무관한 전체 `PENDING` 목록이며 `requested_at DESC, approval_id DESC`로 정렬한다.
...
| `DocumentSearchResponse` | `query:str`, `hits:list[DocumentHit]`, `count:int` |
| `DocumentDetailResponse` | `document_id:str`, `title:str`, `doc_type:SPEC\|MANUAL\|TROUBLESHOOT\|null`, `model_code:str\|null`, `source_path:str\|null`, `version:str\|null`, `chunks:list[{chunk_id:str,chunk_seq:int,section_title:str\|null,content:str}]` |

Neo4j에 존재하지 않는 ID와 PostgreSQL에 존재하지 않는 document_id는 404다. 검색 결과 0건은 오류가 아니며 HTTP 200과 빈 hits를 반환한다.

API의 `document_id`는 배포 원본의 `document.doc_id`와 `document_chunk.doc_id`에 대응한다. DB CHECK가 허용하는 `doc_type`은 대문자 `SPEC | MANUAL | TROUBLESHOOT`이며 null을 허용한다. 현재 배포 문서 3건은 SPEC 2건·TROUBLESHOOT 1건이지만, 현재 행에 MANUAL이 없다는 이유로 허용 Enum에서 제거하지 않는다.

문서 검색은 `score DESC, document_id ASC, chunk_seq ASC`로 안정 정렬한다. `model_code`가 지정되면 해당 모델 문서와 `COMMON` 문서를 함께 검색하고, 생략하면 전체 모델을 검색한다. `top_k` 기본값은 4, 허용 범위는 1~10이며 query는 1~1000자다. 문서 상세의 chunks는 `chunk_seq ASC`다.
...
| API | 주요 입력 | 주요 응답 |
|---|---|---|
| `GET /agent/runs` | status? · equipment_id? · chamber_id? · date_from? · date_to? · page · size | 실행 목록·total |
| `GET /agent/runs/{run_id}` | path | 실행, 전체 alarm_ids, Tool calls, evidence, action, approval |
| `GET /approvals` | status?, page, size | 승인 목록·total |
| `POST /approvals/{approval_id}/decision` | decision(APPROVE/REJECT), decided_by, decision_comment? | 승인·action·agent 상태 |
| `GET /actions` | approval_status? · send_status? · action_code? · equipment_id? · chamber_id? · date_from? · date_to? · page · size | 조치 목록·total |
| `GET /actions/{action_id}` | path | action_history와 delivery 상태 |

`GET /agent/runs`는 `started_at DESC, agent_run_id DESC`, `GET /actions`는 `created_at DESC, action_id DESC`로 정렬한다. 조치 목록의 승인 대기는 별도 엔드포인트가 아니라 `approval_status=PENDING` 필터다.
...
| `IncidentAlarmEvidence` | `alarm_ids:list[str]`, `lot_hist_ids:list[str]`, `rule_ids:list[str]`, `distinct_oos_wafer_count:int`, `distinct_ooc_wafer_count:int`, `has_r03_consec:bool`, `sibling_alarm_counts:dict[str,int]` |
...
| `AgentEvidence` | `representative_fdc:FdcSummaryToolResult\|null`, `r03_fdc:FdcSummaryToolResult\|null`, `incident:IncidentAlarmEvidence`, `equipment_context:EquipmentContextToolResult\|null`, `document_hits:list[DocumentHit]`, `batch_incident_plans:list[BatchIncidentPlan]`, `upstream:list[UpstreamEvidence]`, `errors:list[EvidenceError]` |
...
| `AgentRunDetailResponse` | `agent_run_id:str`, `requested_alarm_id:str`, `representative_alarm_id:str`, `alarm_ids:list[str]`, `alarm_count:int`, `incident:IncidentRef`, `equipment_id:str\|null`, `sensor_id:str\|null`, `recipe_step_name:str\|null`, `incident_first_at:datetime\|null`, `incident_last_at:datetime\|null`, `thread_id:str`, `status:AgentRunStatus`, `fault_code:str\|null`, `cause_summary:str\|null`, `recommended_action:str\|null`, `action_reason:str\|null`, `severity:Severity\|null`, `approval_required:bool`, `confidence:float\|null`, `evidence:AgentEvidence\|null`, `llm_model:str`, `input_tokens:int\|null`, `output_tokens:int\|null`, `latency_ms:int`, `started_at:datetime`, `ended_at:datetime\|null`, `tool_calls:list[ToolCallItem]`, `r03_fdc_evidence:R03EvidenceRef\|null`, `action:ActionDetailResponse\|null`, `approval:ApprovalItem\|null` |
| `ApprovalItem` | `approval_id:str`, `agent_run_id:str`, `action_id:str\|null`, `incident:IncidentRef`, `equipment_id:str\|null`, `sensor_id:str\|null`, `rule_id:str\|null`, `action_code:str`, `severity:Severity\|null`, `requested_at:datetime`, `status:ApprovalStatus`, `decided_by:str\|null`, `decided_at:datetime\|null`, `decision_comment:str\|null` |
...
| `ApprovalDecisionResponse` | `approval_id:str`, `action_id:str`, `approval_status:APPROVED\|REJECTED`, `send_status:SendStatus`, `agent_run_status:AgentRunStatus`, `decided_by:str`, `decided_at:datetime`, `decision_comment:str\|null` |
| `AgentRunItem` | `agent_run_id:str`, `incident:IncidentRef`, `equipment_id:str\|null`, `sensor_id:str\|null`, `recipe_step_name:str\|null`, `alarm_count:int`, `incident_first_at:datetime\|null`, `incident_last_at:datetime\|null`, `started_at:datetime`, `ended_at:datetime\|null`, `status:AgentRunStatus`, `fault_code:str\|null`, `recommended_action:str\|null`, `severity:Severity\|null` |
| `R03EvidenceRef` | `alarm_id:str`, `lot_hist_id:str`, `wafer_no:int`, `sensor_id:str`, `recipe_step_name:str\|null` |
| `ActionItem` | `action_id:str`, `created_by_agent_run_id:str\|null`, `incident:IncidentRef`, `equipment_id:str\|null`, `sensor_id:str\|null`, `recipe_step_name:str\|null`, `action_code:str`, `severity:Severity\|null`, `approval_status:ActionApprovalStatus\|null`, `send_status:SendStatus\|null`, `send_channel:str\|null`, `alarm_count:int`, `created_at:datetime\|null` |
| `ActionDetailResponse` | `action_id:str`, `created_by_agent_run_id:str\|null`, `incident:IncidentRef`, `equipment_id:str\|null`, `sensor_id:str\|null`, `recipe_step_name:str\|null`, `alarm_count:int`, `trigger_alarm_lot_hist_id:str\|null`, `action_code:str`, `reason:str\|null`, `severity:Severity\|null`, `approval_required:bool`, `approval_status:ActionApprovalStatus\|null`, `approved_by:str\|null`, `approved_at:datetime\|null`, `send_channel:str\|null`, `send_status:SendStatus\|null`, `send_started_at:datetime\|null`, `send_attempt_count:int`, `sent_at:datetime\|null`, `created_at:datetime\|null`, `delivery:DeliveryResult\|null` |

`GET /agent/runs`는 `AgentRunItem`, `GET /actions`는 `ActionItem`을 반환한다. 목록에는 `tool_calls`·`evidence`처럼 무거운 필드를 담지 않고 상세 API 에서만 준다. 대신 목록 화면이 정렬·필터에 쓰는 `sensor_id`·`recipe_step_name`·`alarm_count`·구간 시각은 목록 항목에 포함해 상세를 N번 호출하지 않게 한다.

Action의 `created_by_agent_run_id`는 해당 조치를 **최초 생성한 실행**을 가리킨다. 동일 `action_id`를 수동 재실행에서 재사용해도 바꾸지 않으며, incident의 최신 실행을 추측해 반환하지 않는다. 따라서 이 필드는 생성 provenance이고 최신 처리 상태가 필요하면 별도 Agent 실행 목록을 조회한다. 배포 legacy 조치는 null을 허용한다.

**`r03_fdc_evidence`는 `evidence.r03_fdc`와 역할이 다르다.** `evidence.r03_fdc`는 7.2 의 조건부 추가 조회가 담은 `FdcSummaryToolResult` 전문이고, `r03_fdc_evidence`는 그 근거가 **어느 WAFER 에서 나왔는지** 가리키는 참조다. 화면이 근거 패널 제목에 「LOT-260007 W5 · PH_FOCUS · EXPOSE」를 바로 쓰기 위해 필요하며, 대표 알람과 R03 알람의 WAFER 가 다르다는 사실(7.2)을 사용자가 눈으로 확인하는 자리다. 조건부 조회를 하지 않은 실행에서는 둘 다 `null`이다.

API 명세서 v2.0 의 `GET /agent/runs/{run_id}` 에서 `evidence`·`action`·`approval` 블록은 화면이 먼저 쓰는 필드만 적은 **발췌**다. 실제 계약은 이 표의 `AgentEvidence`·`ActionDetailResponse`·`ApprovalItem` 전체이며, 특히 `document_hits`·`equipment_context`는 12.1 의 「증상·위치·지식 세 근거」 화면이 요구하므로 응답에서 빠지지 않는다.

`ApprovalDecisionRequest.decided_by`는 필수를 유지한다. 명세서 v2.0 은 `string | null`로 적었으나 승인·반려는 감사로그의 `actor_id`가 되므로 결정 주체 없는 결정을 허용하지 않는다.

**`IncidentRef`는 `lot_id`와 `chamber_id` 두 개뿐이다(4.1).** 파라미터·구간·집계는 식별 키가 아니므로 `sensor_id`·`recipe_step_name`·`alarm_count`처럼 형제 필드로 올린다. 화면이 파라미터 기준으로 보더라도 incident 식별은 `(lot_id, chamber_id)`로만 한다. 배포 데이터에서는 incident마다 파라미터가 1종이라 결과가 같지만 정의를 섞지 않는다.

`checks`는 Text2SQL 검증 9단계 중 사용자에게 보여줄 항목별 통과 여부다. `valid`가 최종 판정이고 `checks`는 근거 표시용이므로 없어도 계약을 만족한다.
...
| `POST /analytics/validate` | sql | valid, normalized_sql?, reason, checks |
| `GET /analytics/history` | is_valid?, is_rejected?, date_from?, date_to?, page=1, size=20 | 자연어 질의 이력 목록·total |
| `GET /analytics/evaluations` | latest=true, page=1, size=20 | 골드 평가 run 목록과 문항 결과 |
| `GET /audit-logs` | event_type?, actor_type?, entity_type?, entity_id?, date_from?, date_to?, page, size | 감사로그 목록·통계 |

`/analytics/validate`는 검증만 수행하고 SQL을 실행하지 않는다.

**`POST /analytics/query`는 조회지만 `GET`이 아니다.** 요청마다 `nl_query_log` 행을 남겨 멱등하지 않으며, 응답의 `nl_query_log_id`가 그 행을 가리킨다. `GET /analytics/history`는 같은 테이블을 읽기만 하므로 `GET`이다. 이력은 `asked_at DESC, nl_query_log_id DESC`로 정렬하고 `is_rejected=true` 행도 감춘 채 지우지 않는다. 거부·오류 질의를 남겨야 방어 6종이 실제로 동작했음을 화면에서 확인할 수 있다.

정책 위반은 검증기가 안전하게 판정한 정상 처리 결과다. `/analytics/query`는 HTTP 200과 `is_rejected=true`를 반환하고 SQL을 실행하지 않는다. 요청 Body 자체가 잘못됐거나 `question` 길이 제약을 어긴 경우에만 422를 반환한다. LLM·의존성 미준비는 각각 503 `LLM_NOT_READY`·`DEPENDENCY_NOT_READY`다.

평가 이력은 `executed_at DESC, run_id DESC`, 감사로그는 `occurred_at DESC, audit_id DESC`로 정렬한다. `event_type_counts`는 현재 페이지가 아니라 동일 필터가 적용된 전체 결과의 집계다. `event_types`는 화면 필터의 선택지로 쓰는 11장 이벤트 9종 전체 목록이며 필터 결과에 따라 줄어들지 않는다.

`nl_query_log_id`는 DB 컬럼 `nl_query_log.query_id`에 대응한다. 응답 필드는 `agent_run_id` 표기 규칙과 맞춰 테이블명을 붙이고 컬럼명은 원본 스키마를 그대로 둔다(3.1). `AlarmItem.detail`은 `fdc_alarm.detail`이 `text`이므로 구조화 객체가 아니라 문자열이다.
...
| `AnalysisQueryResponse` | `question:str`, `sql:str\|null`, `columns:list[str]`, `rows:list[dict[str,Any]]`, `row_count:int`, `metric:MetricPlan\|null`, `metric_result:int\|float\|list[GroupedMetricResult]\|null`, `group_by:list[str]`, `visualization:VisualizationPlan\|null`, `is_valid:bool`, `is_rejected:bool`, `reject_reason:str\|null`, `error_msg:str\|null`, `latency_ms:int`, `nl_query_log_id:int` |
| `NlQueryLogItem` | `nl_query_log_id:int`, `asked_at:datetime`, `question:str`, `generated_sql:str\|null`, `is_valid:bool`, `is_rejected:bool`, `reject_reason:str\|null`, `row_cnt:int\|null`, `latency_ms:int\|null`, `error_msg:str\|null` |
| `NlQueryHistoryResponse` | `items:list[NlQueryLogItem]`, `total:int`, `page:int`, `size:int` |
| `SqlValidateRequest` | `sql:str(min_length=1,max_length=20000)` |
| `SqlValidateResponse` | `valid:bool`, `normalized_sql:str\|null`, `reason:str`, `checks:list[{key:str,label:str,ok:bool}]\|null` |
...
| `AuditLogResponse` | `items:list[AuditLogItem]`, `total:int`, `page:int`, `size:int`, `event_types:list[str]`, `event_type_counts:dict[str,int]` |
...
`/`는 `/dashboard`로 리다이렉트한다. `/alarms/:alarmId`는 알람 화면의 상세 상태이며 별도 화면으로 세지 않는다.

멘토링 피드백을 반영해 v1.7에서 화면 구성을 바꾸었다. 핵심은 **챔버 상태가 아니라 파라미터 기준으로 본다**는 것이다.

| 변경 | 내용 |
|---|---|
| `운영 대시보드` → `알람 대시보드` | 챔버 상태 카드·계측 PASS율·활성 조치 KPI를 빼고 승인 대기 리스트·기간 추이·파라미터 TOP·설비별 건수를 싣는다 |
| `/traces/:lotHistId` → `/traces` | WAFER 한 장이 아니라 조회 조건으로 본다. x축을 시간으로 바꾸고 파라미터를 다중 선택한다 |
| `/actions` 신설 | 조치를 목록으로 본다. 승인 대기는 이 목록의 기본 필터이며 별도 화면이 아니다 |
| `/approvals` → `/agent-runs/:runId` | 승인은 실행 근거를 본 뒤 내리는 결정이므로 실행 상세 화면에 둔다 |
| `/relations` → `/knowledge` | 단독 화면을 유지하되 Agent 실행 화면이 같은 근거를 함께 보여준다 |

| 화면 | 주요 컴포넌트 | 호출 API | 사용자 동작·화면 전이 |
|---|---|---|---|
| 알람 대시보드 | 기간·계층 필터, 승인 대기 리스트, 알람 추이, 파라미터 TOP, 설비별 건수, 최근 알람 | `GET /dashboard/summary` | 자동·수동 날짜 상태 구분, 승인 대기 항목 선택 시 실행 상세 이동, 최근 알람 선택 시 알람 상세 이동 |
...
| 트레이스 뷰어 | AREA·설비·챔버·파라미터 다중 선택, 레시피·LOT·WAFER 다중 선택, 기간, 파라미터마다 차트 1개 | `GET /traces/catalog`, `POST /traces/search` | x축은 시간, 구간 경계에 LOT·WAFER 표시, 알람 목록에서 진입하면 해당 구간을 강조, Empty 안내 |
| 조치 목록 | 상태·설비·챔버·기간 필터, 조치별 알람 수·승인 상태·전송 상태 | `GET /actions` | 승인 대기가 기본 필터, 항목 선택 시 실행 상세 이동 |
| Agent 실행 근거·승인 | 증상(FDC 차트)·위치(Neo4j 관계)·지식(문서 원문 인용) 세 근거를 나란히, 권고 조치, 승인/반려 폼 | `GET /agent/runs`, `GET /agent/runs/{run_id}`, `GET /approvals`, `POST /approvals/{id}/decision` | 항목 선택→근거 로드→결정, 처리/EXPIRED 409, 성공 후 목록·상세 동시 갱신 |
...
| 자연어 분석 | 질문 입력, SQL·표·통계·동적 차트, 질의 이력, 평가 이력 | `POST /analytics/query`, `POST /analytics/validate`, `GET /analytics/history`, `GET /analytics/evaluations` | 실행·재질의, 정책 거부와 실행 오류 구분, 이력에서 과거 질의 다시 실행, Backend 계획 그대로 렌더링 |
...
- Axios 기본 제한시간은 일반 API에 적용하되 `POST /analytics/query`는 LLM 60초 × 최대 2회 교정 재생성과 처리 여유를 포괄하도록 요청별 `timeout=150000ms`를 사용한다. 10초 공통 제한시간으로 Text2SQL 요청을 먼저 중단하지 않는다.
...
8. `infra/bootstrap/source-data-manifest.json` v2의 16개 base table 원본 컬럼 목록·행 수·canonical content hash와 두 DB를 비교한다. manifest는 `format_version`·`hash_algorithm`·단일 `source.tables` 기준값을 가지며 runtime·evaluation이 공유한다. 검증 연결은 read-only transaction과 30초 statement timeout을 사용한다. 평가 DB는 `action_history=10`을 포함한 초기 기준값을 유지하고, Agent E2E runtime 테이블 초기화 후 운영 DB의 입력 기준 건수도 별도 절차로 검증한다
...
source manifest 생성은 migration 적용 전의 승인된 원본 적재 DB에서만 수행한다. 최초 생성과 변경 모두 `--generate --confirm`이 필요하며 `--confirm`이 없으면 diff 미리보기만 출력하고 파일을 쓰지 않는다. manifest에 기록한 원본 컬럼이 사라지거나 형식·해시 알고리즘 버전이 다르면 DB 조회 또는 평가를 시작하지 않는다. 생성·검증 로그에는 host 별칭·port·DB명만 남기고 사용자명·비밀번호·전체 DSN은 남기지 않는다.
...
- Backend: fresh 구축에서는 bootstrap 성공 marker 이후 시작하되, 일반 기동에서는 외부 의존성 상태를 시작 조건으로 삼지 않는다. PostgreSQL·Neo4j·n8n이 장애 상태여도 FastAPI는 기동하며, 해당 기능 API만 503으로 격리한다(NFR-16).
- Frontend: Backend 컨테이너 기동 후 시작, 자체 `/` healthcheck 제공
...
- Backend는 최초 모델 사용 시 manifest의 feature·버전·SHA-256을 검증한다. 불일치하면 `/summaries/{lot_hist_id}`는 HTTP 503 `MODEL_NOT_READY`, `get_fdc_summary`는 `{ok:false, reason}`을 반환한다. 다른 기능 API는 영향을 받지 않는다.
...
- Compose는 `./backend/model-cache/bge-m3:/models/bge-m3:ro`로 마운트하고 Backend는 `/models/bge-m3`만 읽는다. 모델 경로 또는 query/corpus manifest 검증에 실패하면 `/documents/search`는 HTTP 503 `MODEL_NOT_READY`, `search_documents`는 `{ok:false, reason}`을 반환한다. 다른 기능 API는 영향을 받지 않는다.
...
| 대시보드 기본 조회 | 기간·계층 생략 시 2026-06-01~06-04, reference_date 2026-06-04, 알람 51·OOS 37·OOC 14, 추이·파라미터 상위 5·설비별 건수·최근 알람 5·전체 PENDING 목록이 기준 SQL 및 안정 정렬과 일치 |
...
| fresh Compose | 새 volume 전체 건수·role별 허용/거부·n8n workflow/secret smoke 통과, restart 재적재 0회 |
...
## 18. v1.10 최종 설계 결정 요약

- React UI는 요구사항 v1.9의 8개 업무 화면으로 고정하고 알람 상세·조치 목록·Agent 실행 상세·승인 큐의 책임을 분리한다.
- Text2SQL 정책 거부는 SQL을 실행하지 않은 정상 안전 결과로 HTTP 200 구조화 응답을 반환하고, 요청 형식 오류만 422로 처리한다. Tool의 `POLICY_REJECTED:` 실패 계약은 유지한다.
- 조치 생성 provenance는 별도 migration의 `action_history.created_by_agent_run_id`에 최초 1회 기록하며, 재실행에서 같은 action을 재사용해도 갱신하지 않는다. legacy 조치는 null을 허용한다.
- readiness 의존성별 결과에는 정규화된 `reason`을 포함하고, 원본 접속 오류·주소·계정은 응답에 노출하지 않는다.
- `ET_REFL.upper_only`는 하한값 null 여부가 아니라 명시적 센서 메타데이터 규칙으로 산출한다.
...
- 모델·LLM 미준비는 해당 기능 API 503으로 격리하며 다른 기능과 FastAPI 프로세스에 영향을 주지 않는다.
