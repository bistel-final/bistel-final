...
> [!CAUTION]
> 이 문서는 `kosa_0813` 기준의 이전 epoch 이력이다. 신규 구현·migration 근거로 사용하지 않으며,
> 현재 기준은 `시스템설계서_v2_1_작업본.md`와 최종 패키지 기준표다.
...
>
> 기준 데이터: `kosa_0813.zip` (SHA-256 `8bbe0bdd646290e2da300db0c293d6775927f61a35375721d4b042b239803c96`)
>
...
```mermaid
flowchart LR
    SRC[kosa_0813.zip] --> CORR[Correction/Bootstrap]
...
파일 전달 무결성과 DB bootstrap 기대 상태를 하나의 table hash로 비교하지 않는다. 제공 `action_history` 48건은 source·corrected 파일에는 남지만 Runtime DB에는 적재하지 않기 때문이다.

| artifact | 내용 | `action_history` |
|---|---|---:|
| source file manifest | ZIP·원본 파일별 columns·row_count·content_hash | 제공 Mock 48건 |
| corrected file manifest | 허용된 보정 후 파일별 columns·row_count·content_hash·diff | 제공 Mock 48건 보존 |
| `runtime` bootstrap profile | 실제 적재 대상 table·기대 초기 상태·Runtime 빈 상태 | 0건 |
| `evaluation` bootstrap profile | immutable Text2SQL·화면 질의 snapshot | `fixture_type=MOCK` 48건 |

네 artifact는 각각 `format_version=3`, `artifact_type`, `dataset_epoch=kosa_0813`, source ZIP SHA-256, `correction_version`, `hash_algorithm`, table별 포함 여부·columns·row_count·content_hash를 기록하고, DB bootstrap manifest는 `profile=runtime|evaluation`을 추가한다. 추가 curated fixture는 원본·corrected 파일에 섞지 않고 evaluation profile에 `fixture_type`, version, row_count, content_hash를 별도로 기록한다. Mock·curated fixture는 Agent 조치 또는 Fault 정답으로 사용할 수 없다.
...
runtime: action_history 제외 적재(action=0)
  ...
  → 003_agent_run_severity_pair.sql
  → LangGraph checkpoint setup

evaluation: 제공 action_history 48건 적재
...
`kosa_agent`·`kosa_agent_e2e`는 `runtime` profile, `kosa_text2sql`은 `evaluation` profile만 허용한다. 특히 evaluation 초기화는 schema-first로 `action_history`, `nl_query_log`, R03·문서 table과 `v_alarm_event` 정의를 먼저 만든 뒤 제공 Mock 48건을 적재한다. 이 순서로 `action_history=0` guard가 있는 Agent Runtime migration을 evaluation DB에 잘못 적용하지 않으면서도 Text2SQL logging·조회 View를 제공한다. `fixture_type=MOCK`은 `action_history`의 DB 컬럼이 아니라 evaluation profile/manifest metadata다.
...
| `kosa_text2sql` | immutable SQL 평가 snapshot | corrected source + 제공 Mock 48건(`fixture_type=MOCK`), curated fixture는 별도 version 관리 |
...
`backend/migrations/003_agent_run_severity_pair.sql`은 `002` 적용 뒤 같은 runtime 2개 DB에만
명명 CHECK `ck_agent_run_action_severity_pair`를 추가한다. `002`의 조치 CHECK는 한쪽만 채운 행에서
식이 `NULL`이 되어 PostgreSQL 3값 논리로 통과하므로, `((action IS NULL) = (severity IS NULL))`로
그 구멍을 닫는다. `002` 원본은 수정하지 않으며 stage 계보는
`runtime_clean → runtime_guarded → checkpoint_ready`다.

`action_history`는 base DDL이 만든 table을 사용한다. runtime DB에는 제공 CSV 48건을 적재하지 않고 Agent가 새 조치를 저장한다. evaluation DB에는 제공 48건을 화면·SQL Mock fixture로 적재하되 `fixture_type=MOCK`은 profile metadata에만 기록하고 table 컬럼을 추가하지 않는다.
...
- 제공 action 48건은 incident 목록·정답·seed로 사용하지 않는다.
...
| Trace alarm | corrected canonical key·126건 reference 일치 |
| Summary alarm | canonical key·47건, occurred_at NULL 0 |
