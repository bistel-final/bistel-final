...
(tests/unit/test_sql_validator.py) 50건이다.
...
컬럼 검증이 DB 가 아니라 manifest 를 읽는 이유
    ...
    bootstrap manifest 는 실제 적재와 함께 검증된 컬럼 목록을 이미 담고 있다.
    manifest 에 없는 객체(뷰 등)는 컬럼 검증을 건너뛰고 객체 allowlist 만
    적용한다.
...
@lru_cache(maxsize=1)
def _manifest_columns() -> dict[str, frozenset[str]]:
    """bootstrap manifest 에서 table -> 컬럼 집합을 읽는다.

    manifest 가 없거나 손상돼도 검증기는 동작해야 하므로(다른 방어 5종은
    manifest 무관) 빈 dict 로 강등한다. 그 경우 컬럼 검증만 건너뛴다.
    """
    try:
...
def _collect_alias_names(statement: exp.Expression) -> set[str]:
    """SELECT 출력 별칭. ORDER BY alarm_cnt 처럼 별칭 참조를 컬럼으로
    오인하면 안 된다."""
    names: set[str] = set()
...
def _physical_tables(statement: exp.Expression, cte_names: set[str]) -> list[exp.Table]:
    ...
    for table in statement.find_all(exp.Table):
        if not isinstance(table.this, exp.Identifier):
            # FROM dblink(...) 같은 테이블 함수. 함수 검사가 처리한다.
            continue
...
def validate_sql(sql: str) -> ValidationResult:
    ...
    # ── 5. 시스템 카탈로그 (allowlist 보다 먼저 — 사유를 구체화한다) ────
    for table in tables:
