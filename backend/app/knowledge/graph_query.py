...
def load_graph_revision(
    *,
    ...
    database_name = database or os.getenv("NEO4J_DATABASE", "neo4j")
    ...
    if not isinstance(revision, str) or not revision:
        raise RuntimeError("Neo4j graph marker에 graph_revision이 없습니다")
    return revision
...
def _relation_refs(value: Any) -> list[dict[str, Any]]:
    ...
    for item in value:
        if not isinstance(item, Mapping) or not item.get("relation_id"):
            continue
...
def _business_id(label: str, properties: Mapping[str, Any]) -> str:
    ...
    return "+".join(
        f"{key}={_business_value(properties[key])}" for key in sorted(keys)
    )
